...
class TriplaneRenderer(VolumeRenderer):
    def __init__(self, cfg, sample_mode: str = "bilinear") -> None:
        ...
        self.renderer_3d = torch.nn.Sequential(
            nn.Linear(self.cfg.model.unet.model_channels, self.hidden_dim),
            nn.Softplus(),
            nn.Linear(self.hidden_dim, 1 + self.out_feature_dim)
        )

        nn.init.xavier_uniform_(self.renderer_3d[2].weight, 1 / self.gain) 
        nn.init.constant_(self.renderer_3d[2].bias, 4 / self.gain)
    ...
    def forward(self,  cameras: CamerasBase, triplanes: Triplanes):
        ...
        features_for_rendering = self.renderer_3d(
            rays_features.reshape(
                    -1, rays_features.shape[-1])
                ).reshape(
                    *rays_features.shape[:-1], self.out_feature_dim + 1
                )
        ...
        if self.cfg.render.n_pts_per_ray_fine != 0:
            ...
            features_for_rendering = self.renderer_3d(rays_features.reshape(-1, rays_features.shape[-1])).reshape(
                *rays_features.shape[:-1], self.out_feature_dim + 1
            )
